"""GatedMamba model for Hugging Face."""
...
from .configuration_GatedMamba import GatedMambaConfig
...
class GatedMambaEmbeddings(nn.Module):
    def __init__(
        self,
        config: GatedMambaConfig,
        device=None,
        dtype=None,
    ):
        super().__init__()
...
class GatedMambaMixerModel(nn.Module):
    def __init__(
        self,
        config: GatedMambaConfig,
        device=None,
        ...
        self.embeddings = GatedMambaEmbeddings(config, **factory_kwargs)
...
class GatedMambaPreTrainedModel(PreTrainedModel):
    """PreTrainedModel wrapper for GatedMamba backbone."""

    config_class = GatedMambaConfig
    base_model_prefix = "GatedMamba"
...
class GatedMamba(GatedMambaPreTrainedModel):
    """GatedMamba model that can be instantiated using HF patterns."""

    def __init__(self, config: GatedMambaConfig, device=None, dtype=None, **kwargs):
        ...
        self.config = config
        factory_kwargs = {"device": device, "dtype": dtype}
        self.backbone = GatedMambaMixerModel(config, **factory_kwargs, **kwargs)
...
class GatedMambaForMaskedLM(GatedMambaPreTrainedModel):
    """HF-compatible GatedMamba model for masked language modeling."""

    def __init__(self, config: GatedMambaConfig, device=None, dtype=None, **kwargs):
        super().__init__(config, **kwargs)
        factory_kwargs = {"device": device, "dtype": dtype}
        self.GatedMamba = GatedMamba(config, **factory_kwargs, **kwargs)
        self.lm_head = nn.Linear(
            config.d_model,
            self.config.vocab_size,  # Use GatedMamba config as it might have been updated
            bias=False,
            **factory_kwargs,
        )
    ...
    def get_input_embeddings(self):
        return self.GatedMamba.backbone.embeddings.word_embeddings

    def set_input_embeddings(self, value):
        self.GatedMamba.backbone.embeddings.word_embeddings = value
    ...
    def maybe_weight_tie_mamba(self):
        self.GatedMamba.maybe_weight_tie_mamba()
    ...
    def get_decoder(self):
        ...
        return self.GatedMamba

    def set_decoder(self, decoder):
        """Set decoder (backbone) for the model."""
        self.GatedMamba = decoder
...
class GatedMambaForSequenceClassification(GatedMambaPreTrainedModel):
    def __init__(
        self,
        config: GatedMambaConfig,
    ...
    def forward(
        ...
        if self.conjoin_train or (
            ...
            transformer_outputs = self.GatedMamba(
                input_ids[..., 0],
                inputs_embeds=None,
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
            )
            transformer_outputs_rc = self.GatedMamba(
                input_ids[..., 1],
                inputs_embeds=None,
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
